...
class Spawner(val mGameWorld: GameWorld) : Updateable {
    ...
    fun spawnObject() {
        ...
        if (type == 0) {
            var obstacleBody = mGameWorld.physicsWorld.createStaticBody(Vector2(spikesTexture.regionWidth.toFloat(), spikesTexture.regionHeight.toFloat()), Vector2(xBase + xDiff, Constants.GAME_HEIGHT - (spikesTexture.regionHeight / 2 + 69f)), 100f, Constants.FATAL_OBSTACLE_PHYSICS_TAG, false)
            var obstacle: Obstacle = Obstacle(obstacleBody, spikesInvTexture, speed, this)
            obstacleBody.userData = obstacle
            objects.add(obstacle)
        } else if (type == 1) {
            var obstacleBody = mGameWorld.physicsWorld.createStaticBody(Vector2(spikesTexture.regionWidth.toFloat(), spikesTexture.regionHeight.toFloat()), Vector2(xBase + xDiff, spikesTexture.regionHeight / 2 + 69f), 100f, Constants.FATAL_OBSTACLE_PHYSICS_TAG, false)
            var obstacle: Obstacle = Obstacle(obstacleBody, spikesTexture, speed, this)
            obstacleBody.userData = obstacle
            objects.add(obstacle)
        } else if (type == 2 or 3) {
            var obstacleBody = mGameWorld.physicsWorld.createDynamicBody(Vector2(bombTexture.regionWidth.toFloat(), bombTexture.regionHeight.toFloat()), Vector2(xBase + xDiff, Constants.GAME_HEIGHT / 2 - bombTexture.regionHeight / 2), density, Constants.FATAL_OBSTACLE_PHYSICS_TAG)
            var obstacle: Obstacle = Obstacle(obstacleBody, bombTexture, speed, this)
            obstacleBody.userData = obstacle
            objects.add(obstacle)
        } else if (type == 4 or 5) {
            var obstacleBody = mGameWorld.physicsWorld.createDynamicBody(Vector2(obstacleTexture.regionWidth.toFloat(), obstacleTexture.regionHeight.toFloat()), Vector2(xBase + xDiff, Constants.GAME_HEIGHT / 2 - obstacleTexture.regionHeight / 2), density, Constants.NONFATAL_OBSTACLE_PHYSICS_TAG)
    ...
    fun createCoin(xPos: Float, yPos: Float): Coin {
        var coinBody = mGameWorld.physicsWorld.createStaticBody(Vector2(48f, 48f), Vector2(xPos, yPos), 1f, Constants.COIN_PHYSICS_TAG, true)
